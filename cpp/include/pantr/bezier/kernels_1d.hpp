...
/// ## The name
///
/// This file was `bezier/bezier.hpp` until the directory was about to gain a
/// `Bezier` type. Two headers called `bezier` side by side is a trap, so the
/// kernels took the specific name and the type keeps the generic one. Everything
/// here also moved from `namespace pantr` into `namespace pantr::bezier`, which
/// is what `pantr::grid`, `pantr::geometry` and `pantr::transform` already do.
/// `bezier/bezier.hpp` survives as a one-line forwarder for the installed header
/// set; nothing in this tree should include it.
