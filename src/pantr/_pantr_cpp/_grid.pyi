...
class HierarchicalGrid:
    """Hierarchy of nested tensor-product grids, owned by the C++ core.

    Wrapped by :class:`pantr.grid.HierarchicalGrid`, which is the class a caller holds;
    this one is reached only through it. The wrapper owns the ``ArrayLike`` coercion,
    the scalar-to-per-axis broadcast of ``factor``, the ``TypeError`` for a non-integer
    index argument, the read-only flag on every array it hands out, the ``repr``, the
    pickle, and the memo that makes ``g.root`` one object rather than a fresh copy per
    read.

    One difference is behavioural rather than cosmetic and the wrapper owes it:
    ``cell_id`` raises ``ValueError`` on a ``midx`` of the wrong length, where the
    oracle folds that case into its ``None``. ``is_active_leaf`` is deliberately not
    bound for the same reason -- the wrapper routes it through ``cell_id`` so the length
    rule has one spelling.

    ``cell_tags``, ``facet_tags`` and ``cell_bvh`` return REFERENCES into the grid and
    are bound ``nb::rv_policy::reference_internal``. They alias the grid's own members
    and are valid only while it is alive. ``root`` does not: it hands back a copy.

    Attributes:
        ndim (int): Number of axes.
        num_cells (int): Total active leaf count.
        root (TensorProductGrid): A copy of the level-0 grid.
        factor (tuple[int, ...]): Per-axis subdivision factor, each ``>= 1``.
        max_level (int): Index of the deepest level; ``0`` before any refinement.
    """

    def __init__(self, root: TensorProductGrid, factor: npt.NDArray[np.int64]) -> None: ...
    @staticmethod
    def from_blocks(
        ...
        *,
        ...
        level_start: npt.NDArray[np.int64],
    ) -> HierarchicalGrid: ...
    @property
    def ndim(self) -> int: ...
    @property
    ...
    def active_leaf_mask(self, level: int) -> npt.NDArray[np.uint8]: ...
    def subdomain_mask(self, level: int) -> npt.NDArray[np.uint8]: ...
    def cell_multi_index(self, cid: int) -> tuple[int, ...]: ...
    def cell_id(self, level: int, midx: npt.NDArray[np.int64]) -> int | None: ...
    def export_cells(self) -> tuple[npt.NDArray[np.float64], npt.NDArray[np.int64]]: ...
    def refine(
        self, level: int, lo: npt.NDArray[np.int64], hi: npt.NDArray[np.int64]
    ) -> HierarchicalGrid: ...
    def refine_cells(self, cell_ids: npt.NDArray[np.int64]) -> HierarchicalGrid: ...
    def coarsen(
        self, level: int, lo: npt.NDArray[np.int64], hi: npt.NDArray[np.int64]
    ) -> HierarchicalGrid: ...
    def coarsen_cells(self, cell_ids: npt.NDArray[np.int64]) -> HierarchicalGrid: ...
    def cell_bounds(self, cid: int) -> tuple[npt.NDArray[np.float64], npt.NDArray[np.float64]]: ...
    def locate(self, pt: npt.NDArray[np.float64]) -> int | None: ...
    def neighbor_across_facet(self, cid: int, lfid: int) -> int | None: ...
    def cell_level(self, cid: int) -> int: ...
    ...
    def restrict(
        self, cell_ids: npt.NDArray[np.int64]
    ) -> tuple[HierarchicalGrid, npt.NDArray[np.int64], npt.NDArray[np.uint8]]: ...
    def cell_aabb(self, cid: int) -> AABB: ...
    def child_cells(self, cid: int) -> list[int]: ...
    def reference_map(self, cid: int) -> AffineTransform: ...
    def neighbors(self, cid: int) -> list[int]: ...
