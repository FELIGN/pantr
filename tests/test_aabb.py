...
Ported from a downstream consumer's AABB suite and extended for general ``ndim`` (PaNTr is not
...
def test_aabb_allows_general_ndim() -> None:
    # PaNTr is general-d: a 4-D box is valid (the originating suite capped this at 2/3).
...
def test_aabb_unbounded_allows_high_ndim() -> None:
    # General-d: unbounded works for ndim > 3 (the originating suite rejected this).
    u = AABB.unbounded(5)
    assert u.ndim == 5
