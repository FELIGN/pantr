- [Active task](active-task.md) — bezier's arithmetic merged; two bezier blocks left, and the one gap this port declared rather than closed.
- [Where the decisions live](decisions-pointer.md) — the C++ port's design decisions are in `design/*.md`; four modules merged, nothing in flight. Read backend_parity.md before proposing anything about a bound.
...
- [Reviewing my own measurements](reviewing-my-own-measurements.md) — my prose fails far more often than my code, four cycles running; the newest failure is justifications, twice refuted by evidence already in the repo.
